...
test('cutout alpha preserves original pixels and holes, rejects invalid geometry', async () => {
  const original = await sharp({ create: { width: 100, height: 100, channels: 4, background: { r: 23, g: 95, b: 160, alpha: 0.5 } } }).png().toBuffer();
  const mask = { found: true, rings: [[[100,100],[900,100],[900,900],[100,900]], [[400,400],[600,400],[600,600],[400,600]]], note: '' };
  ...
  await expect(renderGarmentCutout(original, { ...mask, found: false, rings: [] })).rejects.toThrow('No unambiguous');
  await expect(renderGarmentCutout(original, { ...mask, rings: [[[0,0],[1001,0],[0,1000]]] })).rejects.toThrow();
  await expect(renderGarmentCutout(original, { ...mask, rings: [[[10,10],[10,10],[10,10]]] })).rejects.toThrow('coverage');
});

test('live garment segmentation candidate records bounded spend and a reviewable PNG', async ({}, testInfo) => {
  test.skip(!process.env.LIVE_CUTOUT_PHOTO || !process.env.LIVE_CUTOUT_DESCRIPTION, 'Opt-in visual quality evaluation only.');
  loadEnvConfig(process.cwd());
  const db = new PrismaClient();
  const user = await db.user.create({ data: { username: `qa_cutout_${randomBytes(8).toString('hex')}`, passwordHash: 'disabled-evaluation', recoveryHash: 'disabled-evaluation' } });
  ...
  try {
    ...
    else await ledger.settle(user.id, requestId, { state: 'succeeded', actualMicros: result.cost, inputTokens: result.inputTokens, outputTokens: result.outputTokens, result: result.value });
    const cutout = await renderGarmentCutout(photo, result.value);
    await writeFile('../cutout-evaluation.png', cutout.data);
    const report = { model: 'google/gemini-3.8-flash', elapsedMs: Date.now() - start, costMicros: result.cost, inputTokens: result.inputTokens, outputTokens: result.outputTokens, coverage: cutout.coverage, mask: result.value };

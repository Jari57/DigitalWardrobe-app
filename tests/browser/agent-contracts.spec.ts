...
import { agentRequestSchema, captureResultSchema, validateStylistResult, validateSpotterResult } from '../../src/server/agents/contracts';

test('agent proposals cannot introduce unknown garments, duplicates or omit locks', () => {
  const result = { garmentIds: ['top', 'jeans'], explanation: 'These owned pieces suit your stated occasion.', limitations: [] };
  ...
  expect(() => validateStylistResult({ ...result, garmentIds: ['top', 'top'] }, ['top'], [])).toThrow('Duplicate');
...
test('agent requests reject oversized candidate lists and client-controlled provider settings', () => {
  const input = { agent: 'stylist', candidateIds: ['top'], lockedIds: [], occasion: 'Coffee', aesthetic: 'Minimal' };
  expect(agentRequestSchema.parse(input)).toEqual(input);
  expect(() => agentRequestSchema.parse({ ...input, candidateIds: Array.from({ length: 41 }, (_, i) => `item-${i}`) })).toThrow();
  expect(() => agentRequestSchema.parse({ ...input, model: 'unrestricted', maxTokens: 100000 })).toThrow();
});

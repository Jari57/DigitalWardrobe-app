...
test('Blind Fit preserves locks, explains a real AI outfit, reuses it and hands it to canvas', async ({
  page,
  context,
}) => {
  ...
  try {
    const photo = await sharp({
      create: { width: 80, height: 100, channels: 4, background: '#ffffff' },
    })
      .png()
      .toBuffer();
    for (const [name, category, color] of [
      ['White cotton tee', 'tops', '#ffffff'],
      ['Black straight jeans', 'bottoms', '#111111'],
      ['White sneakers', 'shoes', '#ffffff'],
    ]) {
      ...
      expect(
        (
          await context.request.post('/api/garments', {
            headers,
            data: {
              name,
              category,
              color,
              imageUrl: (await upload.json()).imageUrl,
              brand: '',
              price: null,
            },
          })
    ...
    const input = {
      agent: 'stylist',
      candidateIds: garments.map((g: { id: string }) => g.id),
      lockedIds: [],
      occasion: 'Everyday',
      aesthetic: 'Minimal',
    };
    expect(
      (
        await context.request.post('/api/stylist', {
          headers,
          data: { ...input, candidateIds: [] },
        })
      ).status(),
    ).toBe(400);
    expect(
      (
        await context.request.post('/api/stylist', {
          headers,
          data: { ...input, lockedIds: ['unavailable'] },
        })
      ).status(),
    ).toBe(400);
    expect(
      (
        await context.request.post('/api/stylist', {
          headers,
          data: { ...input, candidateIds: ['unavailable'] },
        })
      ).status(),
    ).toBe(404);
    ...
    try {
      ...
      expect((await stranger.request.post('/api/stylist', { headers, data: input })).status()).toBe(
        404,
      );
    } finally {
    ...
    const repeated = await context.request.post('/api/stylist', {
      headers,
    ...
    await page.route('**/api/stylist', (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({ error: 'Today’s AI allowance is used up.' }),
      }),
    );
    await page.getByRole('button', { name: 'Style with AI', exact: true }).click();

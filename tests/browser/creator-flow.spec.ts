...
test('mobile creator: Blind Fit locks, canvas export, saved look, and reference lifecycle', async ({ page, context }) => {
  ...
  try {
    const photo = await sharp({ create: { width: 240, height: 320, channels: 4, background: '#c29b7f' } }).png().toBuffer();
    for (const [name, category] of [['QA top one', 'tops'], ['QA top two', 'tops'], ['QA jeans', 'bottoms'], ['QA shoes', 'shoes']]) {
      ...
      const saved = await context.request.post('/api/garments', { headers, data: { name, category, imageUrl, brand: '', color: '#c29b7f', price: 0 } });
    ...
    const pixel = await sharp(path!).extract({ left: 300, top: 400, width: 1, height: 1 }).removeAlpha().raw().toBuffer();
    ...
    await page.evaluate(() => {
      ...
      Object.defineProperty(navigator, 'share', { configurable: true, value: async (data: ShareData) => {
        (window as unknown as { qaShare: unknown }).qaShare = { active: navigator.userActivation.isActive, type: data.files?.[0]?.type, text: data.text };
    ...
    expect(await page.evaluate(() => (window as unknown as {qaShare: unknown}).qaShare)).toMatchObject({ active: true, type: 'image/png' });
    ...
    await page.getByRole('button', { name: 'Share', exact: true }).click();
    await expect(page.getByText('Direct sharing isn’t supported here.', {exact:false})).toBeVisible();

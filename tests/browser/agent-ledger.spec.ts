...
test('ledger atomically deduplicates, limits budget, and retains uncertain charges', async () => {
  ...
  const user = await db.user.create({
    data: {
      username: namespace.replaceAll('-', '_'),
      passwordHash: 'test-only-no-login',
      recoveryHash: 'test-only-no-login',
    },
  });
  try {
    const ledger = new AgentLedger(
      db,
      { dailyCapMicros: 20, maxRequestMicros: 10, requestsPerUser: 10 },
      namespace,
    );
    const duplicates = await Promise.all(
      Array.from({ length: 4 }, () => ledger.reserve(user.id, 'same-key-123', request)),
    );
    ...
    await expect(
      ledger.reserve(user.id, 'same-key-123', { ...request, occasion: 'Different' }),
    ...
    expect(
      (await db.agentBudget.findFirstOrThrow({ where: { scope: `${namespace}:global` } }))
        .heldMicros,
    ).toBe(10);
    const competitors = await Promise.allSettled(
      Array.from({ length: 3 }, (_, i) => ledger.reserve(user.id, `another-${i}`, request)),
    );
    ...
    const outcome = {
      ...
      inputTokens: 10,
      outputTokens: 5,
      result: { explanation: 'Test only' },
    };
    await Promise.all([ledger.settle(user.id, id, outcome), ledger.settle(user.id, id, outcome)]);
    const budget = await db.agentBudget.findFirstOrThrow({
      where: { scope: `${namespace}:global` },
    });
    ...
    if (pending?.status === 'fulfilled') {
      await db.agentRequest.update({
        where: { id: pending.value.request.id },
        data: { day: '2000-01-01' },
      });
      expect(await ledger.claim(user.id, pending.value.request.id)).toBe(false);
    }
    const limited = new AgentLedger(
      db,
      { dailyCapMicros: 100, maxRequestMicros: 10, requestsPerUser: 1 },
      namespace + '-user-limit',
    );
    await limited.reserve(user.id, 'user-limit-first', request);
    await expect(limited.reserve(user.id, 'user-limit-second', request)).rejects.toThrow(
      'allowance',
    );
  } finally {
...
test('AI is disabled without explicit budget configuration', () => {
  ...
  try {
    expect(() => configuredAgentBudget()).toThrow('not enabled');
  } finally {

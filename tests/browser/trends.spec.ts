...
test('editions require evidence, valid dates and an honest previous-month state', () => {
  ...
  expect(trendEditionSchema.safeParse({...edition,reviewedAt:'2026-99-99'}).success).toBe(false);
  expect(trendEditionSchema.safeParse({...edition,picks:[{...edition.picks[0],sourceId:'missing'}]}).success).toBe(false);
  expect(trendEditionSchema.safeParse({...edition,sources:[{...edition.sources[0],publishedAt:'2027-01-01'}]}).success).toBe(false);
});

test('monthly edit is shared, sourced and opens styling without paid requests', async ({page,request}) => {
  const response = await request.get('/api/trends');
  expect(response.status()).toBe(200);
  expect(response.headers()['cache-control']).toContain('public');
  expect((await response.json()).edition.picks).toHaveLength(10);
  let paidCalls=0;
  page.on('request',request=>{if(request.method()==='POST'&&/api\/(stylist|discovery|spotter)/.test(request.url()))paidCalls++;});
  await page.goto('/');
  ...
  await expect(edit.getByRole('link',{name:/Vogue/}).first()).toHaveAttribute('href',september.sources[0].url);
  await expect(edit.getByRole('link',{name:'Search shops'}).first()).toHaveAttribute('href',/tbm=shop/);

...
async function request(account, path, method = 'GET', body, status = 200, origin = base) {
  ...
  assert.equal(response.status, status, `${method} ${path}: ${response.status} ${text.slice(0, 200)}`);
...
async function signup() {
  ...
  const result = await request(account, '/api/auth', 'POST', {action:'signup',username:account.username,password}, 201);
...
try {
  ...
  assert.deepEqual(await request(a, '/api/wardrobe'), {garments:[],outfits:[],references:[]});
  await request(a, '/api/garments', 'POST', {}, 403, 'https://example.invalid');
  const invalid = new FormData(); invalid.set('file', new Blob(['not an image'], {type:'image/png'}), 'bad.png');
  await request(a, '/api/uploads', 'POST', invalid, 415);
  const bytes = await sharp({create:{width:40,height:60,channels:4,background:'#c29b7f'}}).png().toBuffer();
  const form = new FormData(); form.set('file', new Blob([bytes], {type:'image/png'}), 'qa.png');
  const {imageUrl} = await request(a, '/api/uploads', 'POST', form, 201);
  await request(b, imageUrl, 'GET', undefined, 404);
  const input = {name:'QA <script> literal text',brand:'',category:'tops',color:'#c29b7f',price:0,imageUrl};
  ...
  const oldCookie = a.cookie, oldCode = a.recoveryCode;
  const recovered = await request(a, '/api/auth', 'POST', {action:'recover',username:a.username,password,recoveryCode:oldCode});
  assert.notEqual(recovered.recoveryCode, oldCode);
  await request({cookie:oldCookie}, '/api/wardrobe', 'GET', undefined, 401);
  await request(a, '/api/auth', 'POST', {action:'recover',username:a.username,password,recoveryCode:oldCode}, 401);
  await request(a, `/api/garments/${garment.id}`, 'DELETE');
  assert.equal((await request(a, '/api/wardrobe')).garments.length, 0);
  console.log(`PASS: ${checks} HTTP checks plus persistence, ownership, zero-price, geometry, wear idempotency and recovery assertions.`);
} finally {
  for (const account of accounts) await request(account, '/api/account', 'DELETE', {password:account.password});

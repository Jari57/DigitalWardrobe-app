...
Verification: production build, TypeScript and formatting passed; six targeted checks passed, covering preference replacement, cross-audience title handling, saved-item preservation, source ingestion contracts/diversity, refresh authorization, and mobile selection-to-save plus feed refresh. UI requests use fixtures; persistence is implemented through the existing owner-scoped upsert but was not retested against production Neon in this environment.

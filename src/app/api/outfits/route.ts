...
export async function POST(request: Request) {
  try {
    ...
    const outfit = await db.$transaction(async (tx) => {
      await ownGarments(
        user.id,
        input.pieces.map((p) => p.garmentId),
        tx,
      );
      if ((await tx.outfit.count({ where: { userId: user.id } })) >= 1000)
        throw new ApiError(409, 'Your lookbook has reached its 1,000-look limit.');
      return tx.outfit.create({
        data: { name: input.name, userId: user.id, pieces: { create: input.pieces } },
        include: { pieces: true },
      });
    });

...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!input.candidateIds.length)
      throw new ApiError(400, 'Add some pieces to your closet first.');
    if (
      input.lockedIds.length > 12 ||
      input.lockedIds.some((id) => !input.candidateIds.includes(id))
    )
      throw new ApiError(400, 'Choose at most 12 locks from the available pieces.');
    ...
    if (candidates.length !== input.candidateIds.length)
      throw new ApiError(404, 'Some pieces are unavailable. Refresh your closet.');
    if (process.env.AI_ENABLED !== 'true')
      throw new ApiError(503, 'AI styling is not enabled on this deployment.');
    const normalized = {
      ...input,
    ...
    const { request: record } = await ledger.reserve(user.id, key, normalized).catch(() => {
      throw new ApiError(429, 'Today’s AI allowance is used up. Random Reveal is still available.');
    });
    if (record.result)
      return json({
        ...validateStylistResult(record.result, normalized.candidateIds, input.lockedIds),
        id: record.id,
      });
    if (!(await ledger.claim(user.id, record.id)))
      throw new ApiError(
        409,
        'This styling request is processing or was interrupted. Change your preferences or try tomorrow.',
      );
    dispatched = { ledger, userId: user.id, id: record.id };
    const result = await withAgentUsage(user.id, record.id, () =>
      styleOwnedWardrobe(candidates, input.lockedIds, input.occasion, input.aesthetic),
    );
    if (result.cost === null)
    ...
    if (
      (await db.garment.count({
        where: { userId: user.id, id: { in: result.value.garmentIds } },
      })) !== result.value.garmentIds.length
    )
      throw new ApiError(409, 'Your closet changed. Refresh before styling again.');
    ...
    if (dispatched) {
      ...
      console.error('Stylist generation failed', {
      ...
      return json(
        {
          error:
            'AI styling could not finish. Your current outfit is unchanged. Random Reveal is still available.',
        },
        503,
      );
    }

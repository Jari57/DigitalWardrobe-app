...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [image, candidates] = await Promise.all([
      ...
      db.garment.findMany({ where: { userId: user.id, id: { in: input.candidateIds } }, select: { id: true, name: true, category: true, color: true }, orderBy: { id: 'asc' } }),
    ]);
    ...
    if (!await ledger.claim(user.id, record.id)) throw new ApiError(409, 'This matching request is processing or was interrupted. Try again tomorrow.');
    dispatched = { ledger, userId: user.id, id: record.id };
    const result = await matchInspiration(image.data, image.mimeType, candidates);
    ...
    if (await db.garment.count({ where: { userId: user.id, id: { in: ids } } }) !== ids.length) throw new ApiError(409, 'Your closet changed. Refresh before using these matches.');
    ...
    if (dispatched) {
      ...
      console.error('Spotter generation failed', { name: error instanceof Error ? error.name : 'UnknownError', requestId: dispatched.id });
      return json({ error: 'Matching could not finish. Your saved pairings are unchanged; you can still edit them manually.' }, 503);
    }

...
export default function BlindFit({ garments, onClose, onUse, initialAesthetic = 'Minimal' }: { garments: Garment[]; onClose: () => void; onUse: (pieces: Piece[]) => void; initialAesthetic?: string }) {
  ...
  function shuffle() {
    ...
    for (const category of categories.filter(category => category !== 'dresses')) {
      if (result.some(item => item.category === category) || (hasDress && ['tops', 'bottoms'].includes(category))) continue;
    ...
    setChosen(result); setRecommendation(null); setError('');
  }
  ...
  async function style() {
    ...
    try {
      ...
      const groups = categories.map(category => garments.filter(item => item.category === category && !locks.includes(item.id)));
      ...
      const result = await api<Recommendation>('/api/stylist', 'POST', { agent: 'stylist', candidateIds: pool.map(item => item.id), lockedIds: locks, occasion, aesthetic });
      const selected = result.garmentIds.map(id => garments.find(item => item.id === id));
      if (selected.some(item => !item) || locks.some(id => !result.garmentIds.includes(id))) throw new Error('Your closet changed. Refresh and try again.');
      setChosen(selected as Garment[]); setRecommendation(result);
    } catch (e) { setError((e as Error).message); }
    finally { setBusy(false); }
  }

  return <Modal title="Blind Fit Challenge" dark onClose={() => { if (!busy) onClose(); }}><div className="stack">
    <div className="blind-intro"><span className="eyebrow">LET YOUR CLOSET COOK</span><h3>A little surprise.<br/>A little styling instinct.</h3><p>Reveal a random look or let AI style your saved pieces. Lock your favorites to keep them in the outfit.</p></div>
    {!garments.length ? <p>Add pieces to your closet to start the challenge.</p> : <>
      <div className="blind-grid">{chosen.map(item => <button disabled={busy} aria-pressed={locks.includes(item.id)} key={item.id} className={locks.includes(item.id) ? 'locked' : ''} onClick={() => { setLocks(previous => previous.includes(item.id) ? previous.filter(id => id !== item.id) : [...previous, item.id]); setRecommendation(null); }}><img src={item.imageUrl} alt={item.name}/><span>{item.name}</span>{locks.includes(item.id) ? <Lock size={16}/> : <Unlock size={16}/>}</button>)}</div>
      ...
      {recommendation && <div className="stack" role="status"><strong>Why this works</strong><p>{recommendation.explanation}</p>{recommendation.limitations.map((note, index) => <small key={index}>{note}</small>)}</div>}
      {chosen.length > 0 && <button disabled={busy} onClick={() => onUse(arrange(chosen))}>Style this on canvas</button>}
    </>}
  </div></Modal>;
}

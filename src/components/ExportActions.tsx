...
async function renderExport(garments: Garment[], pieces: Piece[], caption: string) {
  ...
  for (const piece of [...pieces].sort((a, b) => a.zIndex - b.zIndex)) {
    ...
    await image.decode();
    const width = 1080 * 0.34 * piece.scale;
    const height = 1600 * 0.25 * piece.scale;
    const ratio = Math.min(width / image.width, height / image.height);
    ctx.drawImage(
      image,
      (piece.x / 100) * 1080 + (width - image.width * ratio) / 2,
    ...
    >
      <div className="stack">
        {url && (
          <img
            src={url}
            alt="Your outfit export preview"
            style={{ maxHeight: '45dvh', margin: 'auto', objectFit: 'contain' }}
          />
        )}
        <p>1080 × 1920 PNG. Save it or send it to your favorite app.</p>
        <button className="primary" disabled={sharing} onClick={() => download(file, file.name)}>
          <Download size={16} />
          Download PNG
        </button>
        {canShare ? (
              ...
              navigator
                ...
                .catch((error) => {
                  if (error.name !== 'AbortError')
                    setError('Sharing could not open. Download the PNG instead.');
                })
          ...
          <p>
            Direct sharing isn’t supported here. Download the image, then attach it to your post.
          </p>
        )}
        {error && (
...
export default function ExportActions({
  ...
  async function prepare() {
    ...
    try {
      ...
      setError('We couldn’t prepare this look. Check that its photos load and try again.');
    } finally {
      setBusy(false);
    }
  }
  return (
    <>
      <div className="row">
        <button disabled={disabled || busy} onClick={prepare}>
          <Download size={16} />
          Export PNG
        </button>
        <button disabled={disabled || busy} onClick={prepare}>

...
export class AgentLedger {
  constructor(
    ...
    private readonly namespace = 'agents-v1',
  ) {
    ...
    return this.atomic(async (tx) => {
      ...
      if (existing) {
        if (existing.fingerprint !== fingerprint)
          throw new Error('Request key was already used for different input.');
        return { request: existing, reused: true };
      }
      for (const scope of [globalScope, userScope]) {
        await tx.agentBudget.upsert({
      ...
      const global = await tx.agentBudget.findUniqueOrThrow({
        where: { scope_day: { scope: globalScope, day } },
      });
      const user = await tx.agentBudget.findUniqueOrThrow({
        where: { scope_day: { scope: userScope, day } },
      });
      if (user.requests >= this.policy.requestsPerUser)
        throw new Error('Daily generation allowance reached.');
      if (
        global.heldMicros + global.spentMicros + this.policy.maxRequestMicros >
        this.policy.dailyCapMicros
      )
        throw new Error('Daily AI budget reached.');
      for (const scope of [globalScope, userScope]) {
        await tx.agentBudget.update({
          where: { scope_day: { scope, day } },
          data: {
            requests: { increment: 1 },
            heldMicros: { increment: this.policy.maxRequestMicros },
          },
        });
      }
      const record = await tx.agentRequest.create({
        data: {
          userId,
          key,
          fingerprint,
  ...
  async claim(userId: string, id: string) {
    const changed = await this.db.agentRequest.updateMany({
      where: { id, userId, state: 'reserved', day: new Date().toISOString().slice(0, 10) },
  ...
  async markUncertain(userId: string, id: string) {
    ...
    await this.db.agentRequest.updateMany({
      where: { id, userId, state: 'dispatched' },
  ...
  async settle(
    ...
    z.object({
      ...
      actualMicros: z.number().int().min(0).max(1_000_000_000),
    ...
    return this.atomic(async (tx) => {
      ...
      if (!['dispatched', 'uncertain'].includes(record.state))
        throw new Error('Request has not been dispatched.');
      for (const scope of [record.globalScope, record.userScope]) {
        await tx.agentBudget.update({
          where: { scope_day: { scope, day: record.day } },
          data: {
            heldMicros: { decrement: record.reservedMicros },
            spentMicros: { increment: outcome.actualMicros },
          },
        });
      }

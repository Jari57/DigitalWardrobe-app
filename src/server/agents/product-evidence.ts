      ...
      const req = request(
        ...
        (res) => {
          ...
          if (
            ...
            res.destroy();
            reject(new Error('Product page unavailable'));
            return;
          }
          const chunks: Buffer[] = [];
          let bytes = 0;
          res.on('data', (chunk: Buffer) => {
            ...
            if (bytes > 1_000_000) {
              res.destroy();
              reject(new Error('Product page too large'));
...
function typeIs(value: unknown, type: string): boolean {
  return (Array.isArray(value) ? value : [value]).some(
    (v) => v === type || v === `https://schema.org/${type}` || v === `http://schema.org/${type}`,
  );
}
function samePage(value: unknown, page: string) {
  if (typeof value !== 'string') return false;
  try {
    const a = new URL(value, page),
      b = new URL(page);
    return (
      a.origin === b.origin &&
...
export function parseProductEvidence(
  ...
  const unknown: ProductEvidence = {
    availability: 'unknown',
    checkedAt,
    sourceUrl,
    note: 'Retailer stock and price could not be confirmed.',
  };
  ...
  const offers = Array.isArray(product.offers)
    ...
    : product.offers
      ? [product.offers]
      : [];
  if (offers.length !== 1 || !offers[0] || !typeIs(offers[0]['@type'], 'Offer'))
    return {
      ...unknown,
  ...
  const price =
    ...
    (typeof offer.price === 'string' && /^\d+(\.\d{1,4})?$/.test(offer.price))
      ? Number(offer.price)
      : NaN;
  const currency =
    typeof offer.priceCurrency === 'string' && /^[A-Z]{3}$/.test(offer.priceCurrency)
      ? offer.priceCurrency
      : undefined;
  return {
    availability,
...
export async function productEvidence(url: string): Promise<ProductEvidence> {
  try {
    ...
    return parseProductEvidence(page.html, page.url);
  } catch {
    return {
      availability: 'unknown',
      checkedAt: new Date().toISOString(),
      sourceUrl: url,
      note: 'Retailer page could not be checked. Open the source to confirm details.',
    };
  }
}

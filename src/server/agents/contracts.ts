...
export const agentPolicy = Object.freeze({
  version: '1', maxCandidates: 40, maxImages: 1,
  maxOutputTokens: 1200, requestsPerUserPerDay: 10, automaticRetries: 0,
});

const id = z.string().min(1).max(80);
const note = z.string().trim().min(1).max(500);
const category = z.enum(['tops', 'bottoms', 'outerwear', 'shoes', 'accessories', 'dresses']);
const uniqueIds = z.array(id).max(agentPolicy.maxCandidates).refine(
  values => new Set(values).size === values.length, 'Garment IDs must be unique.',
);
...
export const agentRequestSchema = z.discriminatedUnion('agent', [
  ...
  z.object({ agent: z.literal('stylist'), candidateIds: uniqueIds,
    lockedIds: uniqueIds, occasion: z.string().trim().min(1).max(120),
    aesthetic: z.string().trim().max(120) }).strict(),
  z.object({ agent: z.literal('creator'), outfitId: id,
    tone: z.enum(['playful', 'minimal', 'confident']) }).strict(),
]);

export const captureResultSchema = z.object({
  name: z.string().trim().min(1).max(100), category,
  color: z.string().regex(/^#[0-9a-fA-F]{6}$/),
  uncertaintyNotes: z.array(note).max(4),
}).strict();

export const stylistResultSchema = z.object({
  garmentIds: z.array(id).min(1).max(12), explanation: note,
...
export function validateStylistResult(value: unknown, ownedCandidateIds: readonly string[], lockedIds: readonly string[]) {
  ...
  if (selected.size !== result.garmentIds.length) throw new Error('Duplicate garment in proposed outfit.');
  if (result.garmentIds.some(id => !owned.has(id))) throw new Error('Proposal contains an unavailable garment.');
  if (lockedIds.some(id => !owned.has(id) || !selected.has(id))) throw new Error('Proposal did not preserve locked pieces.');
  return result;
}

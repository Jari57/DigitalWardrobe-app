...
export function checkOrigin(request: Request) {
  ...
  if (!origin || !allowed.has(origin)) throw new ApiError(403, 'This request could not be verified. Reload the page and try again.');
...
export async function readJson<T>(request: Request, schema: z.ZodType<T>): Promise<T> {
  ...
  while (true) {
    ...
    if (total > 65536) { await reader.cancel(); throw new ApiError(413, 'Request is too large.'); }
    chunks.push(value);
  }
  let value: unknown;
  try { value = JSON.parse(Buffer.concat(chunks).toString('utf8')); }
  catch { throw new ApiError(400, 'Request data is not valid JSON.'); }
...
export function handleError(error: unknown) {
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    ...
    if (error.code === 'P2003' || error.code === 'P2034') return json({ error: 'This data changed. Refresh and try again.' }, 409);
  }

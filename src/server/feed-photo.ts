...
import { safeFeedImage } from './trend-feed';
import { publicAddress } from './agents/product-evidence';
import { MAX_UPLOAD_BYTES, normalizeImage } from './images';

export async function fetchFeedPhoto(value: string) {
  const signal = AbortSignal.timeout(8000);
  let current = value;
  for (let hop = 0; hop <= 2; hop++) {
    const safe = safeFeedImage(current);
    if (!safe) throw new Error('Unsupported publisher image');
      ...
      const req = request(
        ...
        (res) => {
          ...
          if (
            ...
            res.destroy();
            reject(new Error('Publisher image unavailable'));
            return;
          }
          const chunks: Buffer[] = [];
          let total = 0;

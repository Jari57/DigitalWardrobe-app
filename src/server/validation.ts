...
export const imageUrlSchema = z.string().regex(/^\/api\/images\/[a-zA-Z0-9_-]{1,80}$/, 'Upload a photo first.');
...
export const outfitSchema = z.object({ name: z.string().trim().min(1).max(100), pieces: z.array(pieceSchema).min(1).max(30) }).strict();
export const referenceSchema = z.object({ name: z.string().trim().min(1).max(100), imageUrl: imageUrlSchema, garmentIds: z.array(z.string().min(1).max(80)).max(30) }).strict();
export const wearSchema = z.object({ date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).refine(value => {
  const date = new Date(`${value}T00:00:00Z`);
  return Number.isFinite(date.getTime()) && date.toISOString().slice(0, 10) === value && value >= '2000-01-01' && date.getTime() <= Date.now() + 86400000;
}, 'Choose a valid date, no later than today.') }).strict();
